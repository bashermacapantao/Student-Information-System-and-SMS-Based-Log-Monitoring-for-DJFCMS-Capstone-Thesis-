...
class Gradefour
{
    ...
    public function fetchgradefour_filter($gradefoursection, $gradefouryear)
    {
        ...
        $query = "SELECT * FROM gradsec g inner join adviser a on g.adviser_id = a.adviser_id WHERE g.grade_level = 'Grade - IV' AND g.section = '$gradefoursection' AND s.year ='$gradefouryear'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row;
            }
    ...
    public function fetchgradefour_section_filter($gradefoursection)
    {
        ...
        $query = "SELECT * FROM gradsec g inner join adviser a on g.adviser_id = a.adviser_id WHERE g.grade_level = 'Grade - IV' AND g.section = '$gradefoursection'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row;
            }
    ...
    public function fetchgradefour_year_filter($gradefouryear)
    {
        ...
        $query = "SELECT * FROM gradsec g inner join adviser a on g.adviser_id = a.adviser_id WHERE g.grade_level = 'Grade - IV' AND s.year = '$gradefouryear'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row;
            }
