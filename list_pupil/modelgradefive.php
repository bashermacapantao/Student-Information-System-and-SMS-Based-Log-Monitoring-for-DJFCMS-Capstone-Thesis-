...
class Gradefive
{
    ...
    { 
        ...
        $query = "SELECT * FROM enroll e inner join pupil p on e.pupil_id = p.pupil_id inner join schoolyear s on e.schoolyear_id = s.schoolyear_id inner join gradsec g on e.gradsec_id = g.gradsec_id inner join adviser a on g.adviser_id = a.adviser_id inner join qrcode q on p.qrcode_id = q.qrcode_id WHERE g.grade_level = 'Grade - V'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row; 
            }  
    ...
    public function fetchgradefive_filter($gradefivesection, $gradefiveyear)
    {
        ...
        inner join schoolyear s on e.schoolyear_id = s.schoolyear_id
        inner join gradsec g on e.gradsec_id = g.gradsec_id
        inner join adviser a on g.adviser_id = a.adviser_id inner join qrcode q on p.qrcode_id = q.qrcode_id WHERE g.grade_level = 'Grade - V' AND g.section = '$gradefivesection' AND s.year ='$gradefiveyear'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row;
            }
    ...
    public function fetchgradefive_section_filter($gradefivesection)
    {
        ...
        inner join schoolyear s on e.schoolyear_id = s.schoolyear_id
        inner join gradsec g on e.gradsec_id = g.gradsec_id
        inner join adviser a on g.adviser_id = a.adviser_id inner join qrcode q on p.qrcode_id = q.qrcode_id WHERE g.grade_level = 'Grade - V' AND g.section = '$gradefivesection'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row;
            }
    ...
    public function fetchgradefive_year_filter($gradefiveyear)
    {
        ...
        inner join schoolyear s on e.schoolyear_id = s.schoolyear_id
        inner join gradsec g on e.gradsec_id = g.gradsec_id
        inner join adviser a on g.adviser_id = a.adviser_id inner join qrcode q on p.qrcode_id = q.qrcode_id WHERE g.grade_level = 'Grade - V' AND s.year = '$gradefiveyear'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row;
            }
