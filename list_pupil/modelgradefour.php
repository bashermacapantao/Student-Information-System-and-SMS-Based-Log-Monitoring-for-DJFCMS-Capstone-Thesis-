...
class Gradefour
{
    ...
    { 
        ...
        $query = "SELECT * FROM enroll e inner join pupil p on e.pupil_id = p.pupil_id inner join schoolyear s on e.schoolyear_id = s.schoolyear_id inner join gradsec g on e.gradsec_id = g.gradsec_id inner join adviser a on g.adviser_id = a.adviser_id inner join qrcode q on p.qrcode_id = q.qrcode_id WHERE g.grade_level = 'Grade - IV'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row; 
            }  
    ...
    public function fetchgradefour_filter($gradefoursection, $gradefouryear)
    {
        ...
        inner join schoolyear s on e.schoolyear_id = s.schoolyear_id
        inner join gradsec g on e.gradsec_id = g.gradsec_id
        inner join adviser a on g.adviser_id = a.adviser_id inner join qrcode q on p.qrcode_id = q.qrcode_id WHERE g.grade_level = 'Grade - IV' AND g.section = '$gradefoursection' AND s.year ='$gradefouryear'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row;
            }
    ...
    public function fetchgradefour_section_filter($gradefoursection)
    {
        ...
        inner join schoolyear s on e.schoolyear_id = s.schoolyear_id
        inner join gradsec g on e.gradsec_id = g.gradsec_id
        inner join adviser a on g.adviser_id = a.adviser_id inner join qrcode q on p.qrcode_id = q.qrcode_id WHERE g.grade_level = 'Grade - IV' AND g.section = '$gradefoursection'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row;
            }
    ...
    public function fetchgradefour_year_filter($gradefouryear)
    {
        ...
        inner join schoolyear s on e.schoolyear_id = s.schoolyear_id
        inner join gradsec g on e.gradsec_id = g.gradsec_id
        inner join adviser a on g.adviser_id = a.adviser_id inner join qrcode q on p.qrcode_id = q.qrcode_id WHERE g.grade_level = 'Grade - IV' AND s.year = '$gradefouryear'";
        if ($sql = $this->conn->query($query)) {
            while ($row = mysqli_fetch_assoc($sql)) {
                $data[] = $row;
            }
